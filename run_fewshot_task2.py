...
from data import make_dataframe_task2, create_task2_multi, make_data, TASK2_LABELS

LOSS_NAME_TO_CLASS = {
    "CosineSimilarityLoss": losses.CosineSimilarityLoss,
    "TripletLoss": losses.TripletLoss,
    "TripletLoss1": losses.TripletLoss
}
SEED=42
TEMPLATE = "The underlying theme is "

IDX_TEMPLATES = ['capacity and resources', 'crime and punishment', 'cultural identity',
       ...
       'policy prescription and evaluation', 'political', 'public opinion',
...
def sentence_generates_achor_multiple1(sentences, labels, input_pair):
...
class RunFewShot:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.output_path = (
            ...
            / "model_anchor"
            / "task2"
            / f"task2-{args.model.replace('/', '-')}-{args.loss}-{args.classifier}-itaration_{args.num_itaration}-batch_{args.batch_size}-{args.exp_name}--{args.sample_sizes}-margin_{args.margin}".rstrip(
                "-"
            )
        )
    ...
    def train(self, df_train, y_train, nr=0) -> SKLearnWrapper:
        ...
        ## Add to loos
        if self.args.loss=='TripletLoss1':
            ...
            for _ in range(self.args.num_itaration):
                ...
                train_examples = sentence_generates_achor_multiple1(np.array(x_train), y_train, train_examples)
                
        ## Add to loos
        elif self.args.loss=='TripletLoss':
            train_loss = self.loss_class(
                model=self.model,
                distance_metric=losses.TripletDistanceMetric.COSINE,
                triplet_margin=self.args.margin,
    ...
    def full_train_sem2(self)  -> None:
        """full training on sem task 2 dataset"""
        # load data
        ...
        if self.args.lang=='mul':
            ...
            under_sampling_list = [165,175,185,195,205,215,225]
        else: 
            lang = [self.args.lang]
            under_sampling_list = [30,35,40,50,60]#[25,30,40,50,55,60,70]
        #load data
        ...
        for i in under_sampling_list:
            # Train the model on the current train split
            df_train, labels_matrix = make_data(df3_train_split, task2_labels, sample=self.args.sample, sample_size=i)
            ...
            if self.args.do_eval=='True':
                metrics = self.eval(classifier,df_val, val_labels_matrix, 'f1', task2_labels, i)
    ...
    def full_predict_task2(self):
        ...
        path_folder = os.getcwd()
        dev_folder = path_folder + '/data/{}/dev-articles-subtask-2/'.format(self.args.pred_lang)
        ...
        file_name = 'large_anchor_task2_all_pred_{}.csv'.format(self.args.pred_lang)
...
def main():
    ...
    if args.do_train=='True':
        print('into trian')
        run_fewshot.full_train_sem2()
        
    if args.do_inference=='True':
        print('into predict')
        run_fewshot.full_predict_task2()
